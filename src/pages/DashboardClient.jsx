...
export default function DashboardClientLayout() {
  ...
  const { user, googleConnected, googleUser, logout } = useAuth();
  ...
  return (
    <div className="flex h-screen bg-gray-50 overflow-hidden">
      ...
      <aside className={`flex flex-col bg-white border-r border-gray-200 transition-all duration-300 ease-in-out flex-shrink-0 z-50 fixed md:static inset-y-0 left-0 ${mobileOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"} w-[260px] md:${open ? "w-[260px]" : "w-16"}`}>
        <div onClick={goHome} className="flex items-center gap-3 px-4 py-5 border-b border-gray-100 min-h-[64px] cursor-pointer hover:bg-gray-50 transition-colors">
          <div className="flex-shrink-0"><Logo /></div>
          <span className={`text-[17px] font-bold text-gray-900 tracking-tight whitespace-nowrap transition-opacity duration-200 ${open ? "opacity-100" : "opacity-0 md:hidden"}`}>Bewerber</span>
        </div>

        <nav className="flex flex-col gap-0.5 px-2 pt-4">
          {NAV.map(({ to, label, Icon, end }) => (
            <NavLink key={to} to={to} end={end} onClick={() => setMobileOpen(false)} title={!open ? label : undefined}
              className={({ isActive }) => `flex items-center gap-2.5 px-3 py-2.5 rounded-lg text-[13.5px] font-medium transition-colors duration-150 cursor-pointer ${isActive ? "bg-gray-100 text-gray-900" : "text-gray-500 hover:bg-gray-50 hover:text-gray-800"}`}>
        ...
        <button onClick={() => { navigate("/dashboard-client/my-applications"); setMobileOpen(false); }} title={!open ? "My applications" : undefined}
        ...
        <div className={`hidden md:flex px-2 pb-2 ${open ? "" : "justify-center"}`}>
          <button onClick={() => setOpen(p => !p)} className="flex items-center gap-2.5 w-full px-3 py-2.5 rounded-lg text-gray-500 hover:bg-gray-100 hover:text-gray-800 transition-colors">
            <IconMenu />
            <span className={`text-[13.5px] font-medium transition-opacity duration-200 ${open ? "opacity-100" : "opacity-0 hidden"}`}>Schließen</span>
          </button>
        </div>

        <div className="px-2 pb-2">
          <div className="relative">
            <button onClick={() => setUserMenuOpen(!userMenuOpen)} className={`flex items-center gap-2.5 w-full px-3 py-2.5 rounded-xl hover:bg-gray-50 transition-colors ${open ? "" : "justify-center"}`}>
              {displayUser?.picture ? (
                <img src={displayUser.picture} alt="" className="w-8 h-8 rounded-full flex-shrink-0 object-cover border border-gray-200" />
              ) : (
                <div className="w-8 h-8 rounded-full bg-indigo-600 flex items-center justify-center text-white text-[11px] font-bold flex-shrink-0">{getInitials(displayUser?.name || displayUser?.email)}</div>
              )}
              <div className={`flex-1 overflow-hidden text-left transition-opacity duration-200 ${open ? "opacity-100" : "opacity-0 md:hidden"}`}>
                <p className="text-[13px] font-semibold text-gray-900 truncate leading-tight">{displayUser?.name || displayUser?.email?.split("@")[0] || "Benutzer"}</p>
              ...
              <div className="absolute bottom-full left-2 right-2 mb-1 bg-white border border-gray-200 rounded-xl shadow-lg py-1 z-50">
                <div className="px-3 py-2 border-b border-gray-100">
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    <IconGoogle />
                    <span className="text-xs font-medium text-gray-700">{googleConnected ? "Google" : "Code d'accès"}</span>
                  </div>
                </div>
        ...
        <div className={`flex flex-col gap-2 px-2 pb-4 pt-2 transition-opacity duration-200 ${open ? "opacity-100" : "opacity-0 md:hidden"}`}>
          <button className="flex items-center gap-2.5 w-full px-3 py-2.5 rounded-lg border border-gray-200 bg-white text-[13px] font-medium text-gray-700 hover:bg-gray-50 transition-colors">
            <div className="w-5 h-5 bg-red-100 rounded-full flex items-center justify-center flex-shrink-0"><IconPlay /></div>
            Tutorial Video
          </button>
          <button className="flex items-center justify-center gap-2 w-full px-3 py-2.5 rounded-lg bg-green-500 hover:bg-green-600 transition-colors text-white text-[13px] font-bold">
            <IconWhatsapp />Kontakt mit dem Entwickler
          </button>
        </div>
      </aside>
      ...
      <div className="flex flex-col flex-1 overflow-hidden min-w-0">
        <div className="flex items-center h-14 px-4 md:px-6 bg-white border-b border-gray-200 flex-shrink-0">
          <button onClick={() => setMobileOpen(p => !p)} className="p-1.5 rounded-lg text-gray-500 hover:bg-gray-100 transition-colors md:hidden">
            <IconMenu />
          </button>
          <div className="md:hidden ml-auto">
            {displayUser?.picture ? (
              ...
              <div className="w-8 h-8 rounded-full bg-indigo-600 flex items-center justify-center text-white text-xs font-bold">{getInitials(displayUser?.name || displayUser?.email)}</div>
            )}
          </div>
        </div>
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 md:p-8 bg-gray-50">
          <Outlet context={{ user, googleConnected, googleUser }} />
        </div>
      </div>
    </div>
  );
